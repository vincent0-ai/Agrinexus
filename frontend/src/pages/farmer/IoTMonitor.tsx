import { Thermometer, Droplets, Activity, Sun,
  AlertTriangle, Check, RefreshCw, Usb, Power } from "lucide-react";
...
export function IoTMonitor() {
  ...
  const { latest, history, alerts, metrics,
     ...
     serialStatus, connectHardware, sendHardwareCommand } = useIoT();
  ...
  const togglePump = () => {
    ...
    setPumpOn(newState);
    sendHardwareCommand(newState ? "PUMP_ON" : "PUMP_OFF");
  };
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <button
              ...
              className={cn("flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-semibold transition-colors",
                serialStatus === "connected" ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
                serialStatus === "error" ? "bg-red-50 text-red-700 border-red-200" :
                "border-border hover:bg-muted text-foreground"
              )}
            >
              <Usb className="w-4 h-4" />
              {serialStatus === "connected" ? "Hardware Connected" :
               serialStatus === "error" ? "Connection Failed" :
               "Connect Hardware"}
            </button>
            <button
              onClick={refresh}
          ...
          {loading ? (
            ...
            <div className="col-span-4 bg-card rounded-2xl border border-border p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Click "Connect Hardware" to read from your Arduino via USB.
              </p>
            </div>
          ) : (
            metrics.map((m) => {
        ...
        {serialStatus === "connected" && (
          <div className="bg-card rounded-2xl border border-border p-6">
            ...
            <div className="flex gap-4">
              <button
                onClick={togglePump}
                className={cn("flex items-center gap-2 px-6 py-3 rounded-xl border text-sm font-semibold transition-all",
                  pumpOn ? "bg-emerald-600 text-white border-emerald-600 shadow-md shadow-emerald-600/20" : "bg-card text-foreground border-border hover:bg-muted"
                )}
              >
