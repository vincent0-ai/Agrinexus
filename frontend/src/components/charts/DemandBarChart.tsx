...
import type { DemandIndex } from "@/hooks/useMarketData";

export function DemandBarChart({ data }: { data: DemandIndex[] }) {
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={data} layout="vertical">
        <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" horizontal={false} />
        <XAxis type="number" tick={{ fontSize: 10 }} tickLine={false} domain={[0, 100]} />
        <YAxis type="category" dataKey="crop_name" tick={{ fontSize: 11 }} tickLine={false} width={72} />
