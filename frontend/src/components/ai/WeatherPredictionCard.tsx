...
import { Sparkles, CloudSun, RefreshCw, ChevronDown, ChevronUp } from "lucide-react";
import { cn } from "@/utils/cn";
import { GREEN } from "@/utils/constants";
import { getWeatherPrediction, type WeatherPrediction } from "@/services/aiService";
...
export function WeatherPredictionCard({ county = "Kiambu", crop = "" }: Props) {
  ...
  const fetchPrediction = async () => {
    try {
      const data = await getWeatherPrediction(county, crop);
      setPrediction(data);
    } catch (e) {
      console.error("Weather prediction failed:", e);
    }
  };
  ...
  const lines = prediction.prediction.split("\n").filter(Boolean);
  ...
  return (
    <div className="rounded-2xl border overflow-hidden"
      style={{
        ...
        background: `linear-gradient(135deg, ${GREEN}05, ${GREEN}10)`,
      }}
    >
      {/* Animated top border */}
      <div className="h-0.5 w-full bg-gradient-to-r from-blue-500 via-emerald-500 to-purple-500" style={{
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-gradient-to-br from-blue-500 to-cyan-400">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">
                {prediction.provider} · {county} County
              </p>
            </div>
          </div>
          <button
